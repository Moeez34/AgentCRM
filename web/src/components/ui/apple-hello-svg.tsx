...
/**
 * Official Apple "hello" screensaver-style vector animation.
 * Uses the exact SVG path coordinates from Apple's official system assets.
 * 
 * Gradient colors:
 * Teal (#5ac8d8) -> Green (#5ec47a) -> Yellow (#f5ce3e) -> Orange (#f0944e) -> Coral/Red (#e8555a) -> Pink (#d94578) -> Purple (#b24dbe) -> Blue (#4e8fe0)
 */
export function AppleHelloSvg({ className }: { className?: string }) {
  // Timing variables (3.5s total transition drawing)
  const durationH = 1.0;
  const durationELLO = 2.2;

  return (
    <div className={`flex items-center justify-center w-full ${className || ""}`}>
      <motion.svg
        xmlns="http://www.w3.org/2000/svg"
        // Padded viewBox to completely prevent stroke clipping on any side (strokeWidth is 56)
        viewBox="-200 -80 2700 880"
        className="w-full max-w-[760px] drop-shadow-[0_12px_60px_rgba(0,0,0,0.85)]"
        initial={{ scale: 0.75, opacity: 0 }}
        animate={{ scale: 1.0, opacity: 1 }}
        transition={{ duration: 4.5, ease: [0.16, 1, 0.3, 1] }}
      >
        <defs>
          {/* UserSpaceOnUse linear gradient to span the entire word from left to right */}
          <linearGradient 
            id="apple-hello-gradient" 
            x1="-109.06069946289062" 
            y1="0" 
            x2="2395" 
            y2="0" 
          ...
          >
            ...
            <stop offset="15%" stopColor="#5ec47a" />
            <stop offset="30%" stopColor="#f5ce3e" />
            <stop offset="45%" stopColor="#f0944e" />
            <stop offset="60%" stopColor="#e8555a" />
            <stop offset="73%" stopColor="#d94578" />
            <stop offset="86%" stopColor="#b24dbe" />
            <stop offset="100%" stopColor="#4e8fe0" />
          </linearGradient>

          {/* Glowing premium drop shadow/blur */}
        ...
        {/* scale(1, -1) and translate(0, -728.156) is the official Apple coordinate mapping */}
        <g transform="translate(0, 728.156005859375) scale(1, -1)" filter="url(#apple-glow)">
          {/* Path 1: "h" */}
          <motion.path
