...
// Log key status for visibility
if (OPENAI_KEY) {
  console.log('[AI Service] OpenAI Key detected. Real calls enabled.');
} else {
  console.log('[AI Service] No OpenAI Key found. Running in safe, FREE mock simulation mode.');
}

/**
 * Custom fetch request to OpenAI API
 */
...
/**
 * Generates text embeddings (1536 dims)
 */
export async function getEmbedding(text: string): Promise<number[]> {
  if (!OPENAI_KEY) {
    // Generate a reproducible mock embedding vector (1536 float values)
...
export async function researchCompany(website: string, companyName: string): Promise<{
  ...
  try {
    // In production, we'd scrape the page content. For now we use LLM reasoning on website/company metadata.
...
export async function scoreLead(
  ...
  if (!OPENAI_KEY) {
    ...
    // Determine mock score based on employee count
